...
for ii in np.arange(nb):
    ...
    if (os.path.isfile(str(elements[ii])+'/'+input)):  
      os.system('cd '+str(elements[ii])+'/ && /home/ponce/program/oncvpsp-4.0.1/src/oncvpsp.x < '+input+' > '+output+' && awk \'BEGIN{out=0};/END_PSP/{out=0} {if(out == 1) {print}};/PSPCODE8/{out=1}\' '+output+' > '+psp8+' &&  awk \'BEGIN{out=0};/END_PSP/{out=0} {if(out == 1) {print}};/PSP_UPF/{out=1}\' '+output+' >  '+upf+' && cd ../' )
